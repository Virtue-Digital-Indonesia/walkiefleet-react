...
import { base64ToUint8Array, getBase64ID, getDeviceID, uint8ArrayToBase64 } from './utils'
...
const Home = () => {
  ...
  const runWebsocket = () => {
    if ('WebSocket' in window) {
      ...
      const ProcessDataexGroups = (grps) => {
        setListGroups(grps)
      }

      const ProcessDataexDevices = (devs) => {
        // list of active users (include you)
        setListUsers(devs.filter(item => getDeviceID() !== item.ID))
      }

      const process_voice_packet = (msg) => {
